...
void ofApp::setupCam(int devID){
    ...
    for(int i=0;i<static_cast<int>(wdevices.size());i++){
        if(wdevices[i].bAvailable){
            ...
            ofLog(OF_LOG_NOTICE, "------cantidad de formatos de: " + ofToString(wdevices[i].formats.size()));

            for(size_t f=0;f<wdevices[i].formats.size();f++){
                ofLog(OF_LOG_NOTICE,"Capture Device format available: %ix%i",wdevices[i].formats.at(f).width,wdevices[i].formats.at(f).height);
            }
        }
    }
